...
function makeStyles(C: ColorTheme) {
  return StyleSheet.create({
    ...
    progressCard: { backgroundColor: C.surface, borderRadius: 24, padding: 20, gap: 14, borderWidth: 1, borderColor: `${C.primary}25`, overflow: "hidden" },
    ...
    segmentRow: { flexDirection: "row", backgroundColor: C.surface, borderRadius: 16, padding: 4, gap: 4, borderWidth: 1, borderColor: C.border },
    ...
    segmentTextActive: { fontFamily: "Inter_500Medium", fontSize: 14, color: C.onPrimary },
    section: { gap: 10 },
    sectionLabel: { fontFamily: "Inter_500Medium", fontSize: 12, color: C.textMuted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 },
    habitCard: { flexDirection: "row", alignItems: "center", gap: 14, backgroundColor: C.surface, borderRadius: 20, padding: 16, borderWidth: 1, borderColor: C.border },
    ...
    checkCircle: { width: 32, height: 32, borderRadius: 16, borderWidth: 2, borderColor: C.border, alignItems: "center", justifyContent: "center" },
    templateCard: { flexDirection: "row", alignItems: "center", gap: 14, backgroundColor: C.surface, borderRadius: 18, padding: 14, borderWidth: 1, borderColor: C.border },
    ...
    templateAddBtn: { width: 32, height: 32, borderRadius: 16, backgroundColor: `${C.primary}18`, alignItems: "center", justifyContent: "center" },
    ...
    emptyBtnText: { fontFamily: "Inter_600SemiBold", fontSize: 15, color: C.onPrimary },
    customCta: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 16, backgroundColor: C.surface, borderRadius: 16, borderWidth: 1, borderColor: `${C.primary}30`, borderStyle: "dashed" },
    ...
    addBtnText: { fontFamily: "Inter_600SemiBold", fontSize: 17, color: C.onPrimary },
  });
}

function HabitCard({ habit, completed, streak, onToggle, onRemove, C, styles }: {
  habit: Habit | typeof SUGGESTED_HABITS[0]; completed: boolean; streak: number;
  onToggle: () => void; onRemove?: () => void; C: ColorTheme; styles: ReturnType<typeof makeStyles>;
}) {
  const scale = useSharedValue(1);
  const animStyle = useAnimatedStyle(() => ({ transform: [{ scale: scale.value }] }));
  const handlePress = () => {
    scale.value = withSequence(withTiming(0.95, { duration: 70 }), withSpring(1, { damping: 6 }));
  ...
  return (
    <Pressable onPress={handlePress} onLongPress={onRemove}>
      <Animated.View style={[styles.habitCard, completed && { borderColor: `${habit.color}50`, backgroundColor: `${habit.color}08` }, animStyle]}>
        ...
        <View style={styles.habitContent}>
          ...
          {streak > 1 && <View style={styles.streakRow}><Ionicons name="flame" size={12} color={C.warning} /><Text style={styles.streakText}>{streak} day streak</Text></View>}
        </View>
...
function TemplateCard({ habit, onAdd, styles }: {
  habit: typeof SUGGESTED_HABITS[0]; onAdd: () => void; styles: ReturnType<typeof makeStyles>;
}) {
  return (
    <Pressable onPress={onAdd} style={({ pressed }) => [styles.templateCard, pressed && { opacity: 0.75 }]}>
      ...
      <View style={styles.templateAddBtn}>
        <Ionicons name="add" size={18} color="#7DBB78" />
      </View>
    </Pressable>
  );
}

function CustomHabitModal({ visible, onClose, onAdd }: {
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="formSheet" onRequestClose={onClose}>
      <View style={[styles.modalContainer, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 24 + (Platform.OS === "web" ? 34 : 0) }]}>
        <View style={styles.modalHeader}>
          ...
          <Pressable onPress={onClose} style={styles.modalClose}><Ionicons name="close" size={22} color={C.textMuted} /></Pressable>
        </View>
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ gap: 24, paddingHorizontal: 24, paddingBottom: 24 }}>
          <View>
            ...
            <TextInput style={styles.modalInput} value={name} onChangeText={setName} placeholder="Morning run, drink water..." placeholderTextColor={C.textDim} autoFocus returnKeyType="next" />
          </View>
          <View>
            <Text style={styles.modalLabel}>Description (optional)</Text>
            <TextInput style={styles.modalInput} value={description} onChangeText={setDescription} placeholder="A short reminder" placeholderTextColor={C.textDim} returnKeyType="done" />
          </View>
          <View>
            <Text style={styles.modalLabel}>Icon</Text>
            <View style={styles.iconGrid}>
              {HABIT_ICONS.map((icon) => (
                <Pressable key={icon} onPress={() => setSelectedIcon(icon)} style={[styles.iconOption, selectedIcon === icon && { backgroundColor: `${selectedColor}25`, borderColor: selectedColor }]}>
        ...
        <View style={{ paddingHorizontal: 24, paddingTop: 8 }}>
          <Pressable onPress={handleAdd} disabled={!name.trim()} style={({ pressed }) => [styles.addBtn, pressed && { opacity: 0.85 }, !name.trim() && { opacity: 0.4 }]}>
            <LinearGradient colors={[C.gradStart, C.gradEnd]} style={styles.addBtnGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
...
export default function HabitsScreen() {
  ...
  const styles = useMemo(() => makeStyles(C), [C]);
  const insets = useSafeAreaInsets();
  const { habits, addHabit, removeHabit, toggleHabitCompletion, isHabitCompletedToday, getHabitStreak } = useApp();
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentInsetAdjustmentBehavior="automatic" showsVerticalScrollIndicator={false}
        contentContainerStyle={{ paddingTop, paddingHorizontal: 24, paddingBottom: Platform.OS === "web" ? 34 + 84 : 100, gap: 20 }}>
        ...
        {habits.length > 0 && (
          <View style={styles.progressCard}>
            <LinearGradient colors={[C.primaryGlow, "transparent"]} style={StyleSheet.absoluteFill} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} />
            ...
            {completedToday === habits.length && habits.length > 0 && <Text style={styles.allDoneText}>All done. You're building something real.</Text>}
          </View>
        )}
        ...
        <View style={styles.segmentRow}>
          {(["my", "templates"] as const).map((tab) => (
            <Pressable key={tab} onPress={() => { setActiveTab(tab); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }}
              ...
              <Text style={activeTab === tab ? styles.segmentTextActive : styles.segmentText}>
        ...
        {activeTab === "my" && (
          habits.length > 0 ? (
            <View style={styles.section}>
              {habits.map((habit) => (
                <HabitCard key={habit.id} habit={habit} completed={isHabitCompletedToday(habit.id)} streak={getHabitStreak(habit.id)}
                  onToggle={() => toggleHabitCompletion(habit.id)} onRemove={() => handleRemove(habit.id, habit.name)} C={C} styles={styles} />
              ))}
              <Text style={styles.longPressHint}>Long-press a habit to remove it</Text>
            </View>
          ) : (
            <View style={styles.emptyState}>
              <View style={styles.emptyIconWrap}><Ionicons name="leaf-outline" size={40} color={C.success} /></View>
              <Text style={styles.emptyTitle}>No habits yet</Text>
              <Text style={styles.emptyDesc}>Browse the Templates tab to add proven healthy habits, or create your own.</Text>
              <Pressable onPress={() => setActiveTab("templates")} style={styles.emptyBtn}><Text style={styles.emptyBtnText}>Browse templates</Text></Pressable>
            </View>
          )
        )}
        ...
        {activeTab === "templates" && (
          <View style={styles.section}>
            {availableTemplates.length > 0 ? (
              <>
                <Text style={styles.sectionLabel}>Tap to add · {availableTemplates.length} available</Text>
                {availableTemplates.map((s) => <TemplateCard key={s.id} habit={s} onAdd={() => handleAddTemplate(s)} styles={styles} />)}
              </>
            ) : (
              <View style={styles.emptyState}>
            ...
            <Pressable onPress={() => setShowCustomModal(true)} style={({ pressed }) => [styles.customCta, pressed && { opacity: 0.8 }]}>
      ...
      <CustomHabitModal visible={showCustomModal} onClose={() => setShowCustomModal(false)} onAdd={addHabit} />
    </View>
  );
}
